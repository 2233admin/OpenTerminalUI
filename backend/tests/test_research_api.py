...
async def test_ingest_endpoint_uses_service(monkeypatch):
    ...
    async def fake_ingest_arxiv(query: str = "cat:q-fin.*", *, max_results: int = 25):
    ...
    response = _client().post(
        "/api/research/ingest",
        json={"query": "cat:q-fin.ST", "max_results": 7},
    )

    assert response.status_code == 200
    assert captured == {"query": "cat:q-fin.ST", "max_results": 7}
